// Durak bot for neighbour-only, left-priority model with jokers + load-the-taker.
...
function cardCost(state, card) {
  if (isJoker(card)) return 1000; // jokers are precious — keep for defence
...
export function chooseBotAction(state, id) {
  ...
  if (idx === state.defenderIndex && !state.takeMode) {
    ...
    if (actions.includes('jokerdefend')) {
      ...
      for (const { pair, i } of undef) {
        const j = jokers.find((jj) => cardColor(jj) === cardColor(pair.attack));
  ...
  // Priority attacker (normal fully-defended OR loading a taker)
  if (actions.includes('attack')) {
    const ranks = new Set();
    for (const pair of state.table) {
      if (!isJoker(pair.attack)) ranks.add(parseCard(pair.attack).rank);
      if (pair.defense && !isJoker(pair.defense)) ranks.add(parseCard(pair.defense).rank);
    }
    const throwable = me.hand
      .filter((c) => !isJoker(c) && ranks.has(parseCard(c).rank) && parseCard(c).suit !== state.trumpSuit)
      .sort((a, b) => cardCost(state, a) - cardCost(state, b));
    if (throwable.length) {
      // When loading a taker, pile on freely; otherwise pressure low-card defenders.
      const defender = state.players[state.defenderIndex];
      if (state.takeMode || defender.hand.length <= 2 || Math.random() < 0.5) {
